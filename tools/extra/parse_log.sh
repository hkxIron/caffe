...
if [ "$#" -lt 1 ]
then
echo "Usage parse_log.sh /path/to/your.log /out_parse_path/"
exit
elif [ "$#" -eq 2 ]
then
OUT_PATH="$2"
...
# Generating
echo '#Iters Seconds TestAccuracy TestLoss'> $OUT_PATH/$LOG.test
paste aux0.txt aux4.txt aux1.txt aux2.txt | column -t >> $OUT_PATH/$LOG.test
...
# Generating
echo '#Iters Seconds TrainingLoss LearningRate'> $OUT_PATH/$LOG.train
paste aux0.txt aux3.txt aux1.txt aux2.txt | column -t >> $OUT_PATH/$LOG.train
